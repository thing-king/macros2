...
import compiler/[options, modulegraphs]
import compiler/[
  modules,
  parser, renderer
]
...
proc hint(parts: varargs[string], n: NimNode) =
  hint(parts)
...
template `[]`*(n: NimNode, i: BackwardsIndex): NimNode = n[core.len(n) - i.int]
...
template `^^`(n: NimNode, i: untyped): untyped =
  (when i is BackwardsIndex: core.len(n) - int(i) else: int(i))
